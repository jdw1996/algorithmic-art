	...
	for (let j = 0; j < GRADIENT_ARRAY.length; ++j) {
		for (let k = 0; k < bolts.length; ++k) {
			...
			for (let i = 0; i < currentBolt.points.length - 1; ++i) {
				...
				stroke(lerpColor(background_colour, lightning_colour, GRADIENT_ARRAY[j][1]));
				line(
					cos(currentBolt.points[i][0]) * currentBolt.points[i][1] + centreX,
					sin(currentBolt.points[i][0]) * currentBolt.points[i][1] + centreY,
					cos(currentBolt.points[i+1][0]) * currentBolt.points[i+1][1] + centreX,
					sin(currentBolt.points[i+1][0]) * currentBolt.points[i+1][1] + centreY
				);
			}
		}
	}
